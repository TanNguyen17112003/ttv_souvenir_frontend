...
function OrderDetail() {
    ...
    const handleRemoveOrder = async () => {
        await axios
            .delete(`http://localhost:3400/order/${idOrder}`)
            .then((response) => {
    ...
    const handleConfirmOrder = async () => {
        await axios
            .post(`http://localhost:3400/order/${idOrder}/${couponPrice}`)
            .then((response) => {
        ...
        const getDetailOrder = async () => {
            try {
                const result = await axios.get(
                    `http://localhost:3400/order/product/${idOrder}`
                );
    ...
    useEffect(() => {
        const getCouponCost = async () => {
            try {
                const result = await axios.get(
                    `http://localhost:3400/coupon/couponCost/${idOrder}`
                );
    ...
    useEffect(() => {
        ...
        const newTotalCost = productDetail.reduce(
            (acc, product, index) =>
                acc + product.GiaBan * currentNumbers[index],
            0
        );
    ...
    useEffect(() => {
        const getStatus = async () => {
            try {
                const result = await axios.get(
                    `http://localhost:3400/order/status/${idOrder}`
                );
    ...
    return (
        <WithWrap>
            {!userEmail ? (
                <h1>
                    Bạn vui lòng đăng nhập để theo dõi chi tiết đơn hàng của bạn
                </h1>
            ) : (
                <div className="flex justify-center">
                    {productDetail.length > 0 ? (
                      <div className="p-10">
                        <div className="mb-10 flex gap-10 w-[100%] justify-around">
                            ...
                            <div className="p-5 border-2 border-slate-400 rounded-lg shadow-xl hover:shadow-2xl h-[300px]">
                                <h2 className="text-center uppercase text-blue-400 font-bold mb-5 text-[28px]">
                                    hóa đơn
                                </h2>
                        ...
                        <SidebarAdoptCoupon
                                status1={showAdoptCoupon}
                                totalPrice={totalPrice}
                                idOrder={idOrder}
                                handleAdoptVoucher={handleAdoptVoucher}
                        />
                      </div>
                        
                    ) : (
                        `Đơn hàng ${idOrder} không tồn tại trên hệ thống`
                    )}
                </div>
            )}
        </WithWrap>
