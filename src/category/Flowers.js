...
function Flowers() {
    ...
    return (
        <div className='px-[20px] py-[10px] grid grid-cols-4 '>
            {products.map((item) => (
                    <Link className=' hover:border-slate-400 flex flex-col justify-center items-center border-2 border-solid border-slate-300  h-[400px]'key={item.id} to={`../items/${item.id}`}>
